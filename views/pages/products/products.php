...
if ($totalProducts->status == 200) {
	...
	if ($totalProducts->status == 200) {
		...
		if ($routesArray[0] == "free") {
			...
			/*=============================================
			Filtrando productos ondemand
			=============================================*/
		} else if ($routesArray[0] == "ondemand") {
			$url = "relations?rel=variants,products&type=variant,product&linkTo=type_variant&equalTo=ondemand&select=id_product";
			$totalProducts = CurlController::request($url, $method, $fields);
			...
			if ($totalProducts->status == 200) {
				...
				$select = "id_product,name_product,url_product,type_variant,media_variant,date_created_product,description_product";
				$url = "relations?rel=variants,products&type=variant,product&linkTo=type_variant&equalTo=ondemand&startAt=" . $startAt . "&endAt=" . $endAt . "&orderBy=id_variant&orderMode=DESC&select=" . $select;
				$method = "GET";
				$fields = array();
    ...
    <div class="container">
            ...
            <div class="col px-3 py-3">
                ...
                <a href="/<?php echo $value->url_product ?>">
                    ...
                    <figure class="imgProduct">
                        ...
                        <?php if ($value->type_variant == "gallery" || $value->type_variant == "ondemand") : ?>

                        <img src="<?php echo $path ?>views/assets/img/products/<?php echo $value->url_product ?>/<?php echo json_decode($value->media_variant)[0] ?>"
                            class="img-fluid">
                ...
                <div class="clearfix">
                    ...
                    <h5 class="float-start text-uppercase text-muted">
                        ...
                        <?php if ($value->offer_variant > 0) : ?>
                        <del class="small" style="color:#bbb">MXN $<?php echo $value->price_variant ?></del>
                        $<?php echo $value->offer_variant ?>
                        <?php else : ?>
                    ...
                    <span class="float-end">
                        ...
                        <div class="btn-group btn-group-sm">
                            ...
                            <button type="button" class="btn btn-light border 
                                ...
                                <?php endif ?> idProduct="<?php echo $value->id_product ?>"
                                idFavorite="<?php echo $value->id_favorite ?>" pageFavorite="no">
                            ...
                            <button type="button" class="btn btn-light border"
                                onclick="location.href='/<?php echo $value->url_product ?>'">
                        ...
                        <span class="float-end">
                            ...
                            <div class="btn-group btn-group-sm">
                                ...
                                <button type="button" class="btn btn-light border 
									<?php if (isset($_SESSION["user"]) && $value->id_favorite == 0) : ?> addFavorite <?php endif ?>
									<?php if (isset($_SESSION["user"]) && $value->id_favorite > 0) : ?> remFavorite <?php endif ?>"
                                    <?php if (!isset($_SESSION["user"])) : ?> data-bs-toggle="modal"
                                    data-bs-target="#login" <?php endif ?> idProduct="<?php echo $value->id_product ?>"
                                    idFavorite="<?php echo $value->id_favorite ?>" pageFavorite="no">
                                    <?php if ($value->id_favorite > 0) : ?>
                                    <i class="fas fa-heart" style="color:#dc3545"></i>
                                    <?php else : ?>
        ...
        <div class="d-flex justify-content-center mt-3 mb-5">
            ...
            <div class="cont-pagination">
                ...
                <ul class="pagination" data-total-pages="<?php echo ceil($totalProducts / $endAt) ?>"
